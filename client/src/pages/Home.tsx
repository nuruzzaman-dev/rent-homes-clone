...
export default function Home() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (feelRef.current && desktopMotion) gsap.to({ progress: 0 }, { progress: 1, ease: "none", scrollTrigger: { trigger: feelRef.current, start: "top 70%", end: "bottom 35%", scrub: 1, onUpdate: (self) => { const index = Math.min(feelWords.length - 1, Math.floor(self.progress * feelWords.length)); setActiveFeel(index); if (feelImageRef.current) feelImageRef.current.src = feelImages[index]; } } });
