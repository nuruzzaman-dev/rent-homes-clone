...
export default function Home() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (feelRef.current && feelImageRef.current && desktopMotion) {
        gsap.to({ progress: 0 }, { progress: 1, ease: "none", scrollTrigger: { trigger: feelRef.current, start: "top top", end: "+=1100", pin: true, pinSpacing: true, scrub: 1, anticipatePin: 1, invalidateOnRefresh: true, onUpdate: (self) => { const index = Math.min(feelWords.length - 1, Math.floor(self.progress * feelWords.length)); if (index === feelIndexRef.current) return; feelIndexRef.current = index; setActiveFeel(index); const image = feelImageRef.current; if (image) { gsap.to(image, { opacity: 0, duration: 0.16, overwrite: true, onComplete: () => { image.src = feelImages[index]; gsap.to(image, { opacity: 1, duration: 0.34, overwrite: true }); } }); } } } });
      }
      ScrollTrigger.refresh();
    }, rootRef);
    return () => { cancelAnimationFrame(id); removeCursorListeners(); ctx.revert(); };
  }, []);
  ...
  return <div ref={rootRef} id="top" className="min-h-screen overflow-x-hidden bg-[#f5f5f1] font-[DM_Sans] text-[#182531] selection:bg-[#3d6f9d] selection:text-white"><div className={`pointer-events-none fixed inset-0 z-[70] flex items-center justify-center bg-[#142331] transition-opacity duration-500 ${loaded ? "pointer-events-none opacity-0" : "opacity-100"}`} aria-hidden="true"><div className="text-center text-white"><p className="font-[Cormorant_Garamond] text-4xl tracking-[-0.04em]">Rent<span className="text-[#8eafc0]">Homes</span></p><p className="mt-3 font-mono text-[9px] uppercase tracking-[0.2em] text-white/50">01 / 01</p></div></div><div ref={cursorRef} data-label="VIEW" className="pointer-events-none fixed left-0 top-0 z-[80] hidden h-3 w-3 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-[#dce8ed] text-[9px] font-semibold uppercase tracking-[0.15em] text-[#182531] after:content-[attr(data-label)] lg:flex" aria-hidden="true" />
    ...
    <main>
      ...
      <section ref={feelRef} className="relative min-h-[100svh] overflow-hidden bg-[#eef1ef] px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="absolute inset-0 opacity-0 transition-opacity duration-700 lg:opacity-20"><img ref={feelImageRef} src={feelImages[0]} alt="" className="h-full w-full object-cover grayscale" /></div><div className="relative mx-auto flex min-h-[calc(100svh-6rem)] max-w-[1440px] flex-col justify-center"><div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end"><div><SectionLabel>10 / What matters</SectionLabel><h2 className="mt-6 max-w-[750px] font-[Cormorant_Garamond] text-6xl font-medium leading-[0.84] tracking-[-0.07em] sm:text-8xl">What makes a home feel right?</h2></div><p className="max-w-[250px] text-sm leading-6 text-[#7b898e]">Not every answer is visible at first glance.</p></div><div className="mt-20 grid border-t border-[#cdd6d3] sm:grid-cols-2 lg:grid-cols-3">{feelWords.map(([word, sub], index) => <button key={word} onClick={() => chooseFeel(index)} className={`group border-b border-[#cdd6d3] px-0 py-8 text-left sm:px-7 lg:px-9 ${activeFeel === index ? "bg-white/30" : ""}`}><span className={`font-mono text-[10px] ${activeFeel === index ? "text-[#3d6f9d]" : "text-[#98a5a7]"}`}>0{index + 1}</span><span className={`mt-8 block font-[Cormorant_Garamond] tracking-[-0.05em] transition sm:text-5xl ${activeFeel === index ? "text-5xl text-[#182531]" : "text-4xl text-[#708087] group-hover:text-[#182531]"}`}>{word}</span><span className={`mt-2 block text-[10px] font-semibold uppercase tracking-[0.15em] text-[#3d6f9d] transition ${activeFeel === index ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>{sub}</span></button>)}</div></div></section>
