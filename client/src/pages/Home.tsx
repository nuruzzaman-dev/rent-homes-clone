...
const lifestyleCards = [
  { number: "01", title: "City life", description: "For people who want everything within reach.", image: lifestyleImages[0], width: "w-[calc(100vw-2rem)] sm:w-[calc(100vw-4rem)] lg:w-[38vw]" },
  { number: "02", title: "Coastal living", description: "For mornings that begin with a horizon.", image: lifestyleImages[1], width: "w-[calc(100vw-2rem)] sm:w-[calc(100vw-4rem)] lg:w-[34vw]" },
  { number: "03", title: "Quiet retreat", description: "For the space to hear yourself think.", image: lifestyleImages[2], width: "w-[calc(100vw-2rem)] sm:w-[calc(100vw-4rem)] lg:w-[40vw]" },
  { number: "04", title: "Modern minimal", description: "For a life shaped by light and line.", image: lifestyleImages[3], width: "w-[calc(100vw-2rem)] sm:w-[calc(100vw-4rem)] lg:w-[32vw]" },
  { number: "05", title: "Nature escape", description: "For the feeling of arriving somewhere rare.", image: lifestyleImages[4], width: "w-[calc(100vw-2rem)] sm:w-[calc(100vw-4rem)] lg:w-[36vw]" },
];
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (mobileMotion && lifestyleRef.current && lifestyleTrackRef.current) {
        const track = lifestyleTrackRef.current;
        const copy = lifestyleRef.current.querySelector<HTMLElement>("[data-mobile-exploration-copy]");
        const travel = () => Math.max(0, track.scrollWidth - window.innerWidth + 24);
        const mobileTimeline = gsap.timeline({
          scrollTrigger: {
            trigger: lifestyleRef.current,
            start: "top top",
            end: () => `+=${Math.max(1500, travel() + window.innerHeight * 0.4)}`,
            pin: true,
            pinSpacing: true,
            scrub: 1.35,
            anticipatePin: 1,
            invalidateOnRefresh: true,
            onEnter: () => { if (copy) gsap.set(copy, { autoAlpha: 1, y: 0 }); },
            onLeave: () => { if (copy) gsap.set(copy, { autoAlpha: 1, y: 0 }); },
            onEnterBack: () => { if (copy) gsap.set(copy, { autoAlpha: 1, y: 0 }); },
          },
        });
        mobileTimeline.to(track, { x: () => -travel(), ease: "none", duration: 1 }, 0);
        if (copy) {
          mobileTimeline.to(copy, { autoAlpha: 0.18, y: -10, ease: "power2.out", duration: 0.12 }, 0.14);
          mobileTimeline.to(copy, { autoAlpha: 1, y: 0, ease: "power2.out", duration: 0.14 }, 0.86);
        }
      }
